...
for i =1 :1000
    j = 0;
j = round(random(pd_np,size(rnk_pks,2),1));
rnd_imp(:,i) = j; 
...
opt_w = [];
while isempty(opt_w)
prompt = {'Threshold:','Min Peak Prominence:','Min Peak Height:'};
...
legend('max delta s based on w'); hold off;
 [p,l, w,pr] = findpeaks(act_p, 'Threshold', peak_input(1),'MinPeakprominence',peak_input(2),'MinPeakHeight',peak_input(3));
 figure('Name', 'Opt window size'); clf; 
plot(act_p);hold on;
...
[~,c] = find( a< opt_w2);
[~,d] = find( a> opt_w2);
indx_comm = setxor(b,c); 
indx_comm= [ 1, indx_comm];
...
num_com = size(d,2)+j;
t_imp= [];
for i = 1: length(indx_comm)-1
    if indx_comm(i + 1) - indx_comm(i) ==1 
            t_imp(i,1) = time(indx_comm(i+1)); 
     t_imp(i,2) = time(indx_comm(i+1));
    else 
     t_imp(i,1) = time(indx_comm(i)+1); 
     t_imp(i,2) = time(indx_comm(i+1));
    end
end
aa = [time(1), time(indx_comm(1))];
t_imp = [aa;t_imp];
...
figure('Name',append('centroid of synchronous cells in event',string(j),'with criterion of', string(criteria)));clf; scatter(sync_roi_loc(1,:), sync_roi_loc(2,:),'green','filled');
...
figure('Name',append('quality metric for event',string(j),'with criterion of', string(criteria)));clf; plot(q_met); hold on; 
...
figure('Name',append('Cell_comm_location',string(j),'with criterion of', string(criteria)));clf; scatter(cell_comm_loc(:,1), cell_comm_loc(:,2),'green','filled');
dx = 0.3; dy = 0.3; % displacement so the text does not overlay the data points
